...
const createNotatedMove= (notatedMoves, src, dest) => {
    const moveKey = Object.keys(notatedMoves).find(key => {
        ...
        const isSameDest = move.dest.file === dest.file && move.dest.rank === dest.rank
...
io.on('connection', (socket) => {
    ...
    socket.on('joinGame', ({ name, room }, onSuccess, onError) => {
        ...
        socket.join(game.id)
        io.in(game.id).emit('startGame', {currentTurn: game.currentTurn, id: game.id, status: game.gameClient.getStatus()});
    })

    socket.on('move', ({src, dest, gameId}, onError) => {
        const { game, error } = movePiece(src, dest, gameId, socket.id);
        if (error) return onError(error)
        io.in(game.id).emit('updateBoard', {currentTurn: game.currentTurn, id: game.id, status: game.gameClient.getStatus()});
    })
